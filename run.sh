...
runCode()
{
  ...
  if [ $testing -eq 0 ]; then
    echo 'Let us train!'
    CUDA_LAUNCH_BLOCKING=1 TORCH_USE_CUDA_DSA=1 python -m examples.nlp train $training_data
  elif [ $testing -eq 1 ]; then
    echo 'Let us train, and log with wandb!'
    CUDA_LAUNCH_BLOCKING=1 TORCH_USE_CUDA_DSA=1 python -m examples.nlp train --wandb True $training_data
  elif [ $testing -eq 2 ]; then 
    echo 'Let us test!'
    read -p "Type what you want the model to work with --> " sampletxt
    CUDA_LAUNCH_BLOCKING=1 TORCH_USE_CUDA_DSA=1 python -m examples.nlp sample --precond "$sampletxt" --num-tokens $num_tokens $sample_model
  else
    echo 'Let us train, and log with wandb!'
    read -p "Type what you want the model to work with --> " sampletxt
    CUDA_LAUNCH_BLOCKING=1 TORCH_USE_CUDA_DSA=1 python -m examples.nlp sample --wandb True --num-tokens $num_tokens --precond "$sampletxt" $sample_model
  fi
}
